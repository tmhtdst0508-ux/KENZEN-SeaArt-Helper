...
Sub WrapLastBlock()
    ...
    If selLen > 0 Then
        ...
        ' ★バグ修正：TextBoxのSelStart(改行1文字換算)と.Text(改行2文字換算)のズレを補正
        Dim textForSelection As String
        textForSelection = Replace(currentText, vbCrLf, vbLf)
        textForSelection = Replace(textForSelection, vbCr, vbLf)
        
        ' 選択開始位置までの改行の数をカウントし、実際の文字インデックスを補正
        Dim tempPrefix As String
        tempPrefix = Left(textForSelection, selStartPos)
        Dim lineBreakCountInPrefix As Long
        lineBreakCountInPrefix = Len(tempPrefix) - Len(Replace(tempPrefix, vbLf, ""))
        
        ' 選択範囲内の改行の数をカウントし、実際の選択文字数を補正
        Dim tempTarget As String
        tempTarget = Mid(textForSelection, selStartPos + 1, selLen)
        Dim lineBreakCountInTarget As Long
        lineBreakCountInTarget = Len(tempTarget) - Len(Replace(tempTarget, vbLf, ""))
        
        ' 補正された正確なインデックス
        Dim realStartPos As Long: realStartPos = selStartPos + lineBreakCountInPrefix
        Dim realLen As Long: realLen = selLen + lineBreakCountInTarget
        
        prefix = Left(currentText, realStartPos)
        Target = Mid(currentText, realStartPos + 1, realLen)
        suffix = Mid(currentText, realStartPos + realLen + 1)
    Else
    ...
    If isSelectionMode Then
        ...
        ' ★ハイライト復元時も新しいターゲット内の改行文字数ズレを補正
        Dim newTargetLenForSel As Long
        newTargetLenForSel = Len(Replace(Replace(newTarget, vbCrLf, vbLf), vbCr, vbLf))
        MainWindow.txtMain.SelLength = newTargetLenForSel
    End If
End Sub
...
' 2. AIへのシステムプロンプト
    ...
    sysPrompt = sysPrompt & "You MUST sort all tags strictly in the following order: Art Style -> Camera Angle -> Character Count -> Character Roles -> Relationship -> Skin & Attributes -> Body Type -> Hair length -> Bangs -> Tying -> Hair Color -> Expressions -> Body Hair -> Eye Color ->" & _
...
Public Sub InitConfigJSON()
    ...
    If Left(ThisWorkbook.Path, 4) = "http" Then
        ...
        ' ★修正：初期化を中止したことによるエラー91と無限ループを防ぐため、ブックを強制終了する
