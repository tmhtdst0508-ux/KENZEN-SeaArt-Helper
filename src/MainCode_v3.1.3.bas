...
Sub WrapLastBlock()
    Dim currentText As String, prefix As String, Target As String
    Dim lastCommaPos As Long, lastLF As Long, lastCR As Long
    ...
    currentText = Trim(MainWindow.txtMain.text)
    If currentText = "" Then Exit Sub
    
    ' 3. 直前のブロックの「本当の」区切り位置を特定
    ' 区切り文字は「カンマ」または「改行」のうち、一番後ろにあるもの
    lastCommaPos = InStrRev(currentText, ",")
    lastLF = InStrRev(currentText, vbLf)
    lastCR = InStrRev(currentText, vbCr)
    
    ' 3つのうち、最も末尾に近い位置を分割ポイントにする
    splitPos = lastCommaPos
    If lastLF > splitPos Then splitPos = lastLF
    If lastCR > splitPos Then splitPos = lastCR
    
    If splitPos > 0 Then
        ' カンマや改行がある場合：そこまでをprefix、それ以降をTargetにする
    ...
    ' すでにカッコとコロンがある場合は解除
    If Left(Target, 1) = "(" And InStr(Target, ":") > 0 And Right(Target, 1) = ")" Then
        Dim colonPos As Long: colonPos = InStrRev(Target, ":")
        ' カッコと「:1.x」を除去して中身だけ取り出す
        Target = Mid(Target, 2, colonPos - 2)
    Else
        ' 重み付け適用
        weight = "1.1" ' デフォルト
        On Error Resume Next
        weight = MainWindow.cmbWeightValue.Value ' コンボボックスから取得
        On Error GoTo 0
    ...
    ' 5. プロンプトの再組み立て
    If prefix <> "" Then
        ' prefixの末尾がカンマの場合のみ、スペースを1つ入れて結合（整形）
        If Right(prefix, 1) = "," Then
            AccumulatedPrompt = prefix & " " & Target
        Else
            ' 改行の直後（BREAKの下など）の場合はスペースを入れずに結合
            AccumulatedPrompt = prefix & Target
        End If
    Else
        AccumulatedPrompt = Target
    End If
    ...
    ' 7. 後処理
    Call UncheckWeightUI ' チェックボックスをオフにする
    
    Application.StatusBar = "Block Weighted: " & Target
...
Private Function IsFileUTF8(ByVal filePath As String) As Boolean
...
Sub InitDictionaries()
    ...
    ' C(日本語)とD(英語)... 奇数列を基点に偶数列をペアにする設計
        colArr = Array("A", "E", "G", "I", "K", "M", "O", "Q", "S", "U", "W", "Y", "AA", "AC", "AE", "AG", "AI", "AK", "AM", "AO", "AQ", "AS", "AU", "AW", "AY", "BA", "BC", "BE", "BG", "BI", "BK", "BM", "BO", "BQ")
...
' 2. AIへのシステムプロンプト
    ...
    sysPrompt = sysPrompt & "You MUST sort all tags strictly in the following order: Character Count -> Skin & Attributes -> Body Type -> Hair length -> Bangs -> Tying -> Hair Color -> Body Hair " & _
                "-> Occupation -> Underwear -> Outfit -> Outfit State -> Headwear -> Hands & Wrists -> Footwear & Legwear -> Accessories -> Location -> Time & Surroundings" & _
                "-> Position -> General Action -> Major Categories of Sexual Act -> Sexual Act -> Bondage Action & Movement -> Means & Props -> Body Parts -> Interaction State -> Expressions -> Body fluids -> Misc Items -> Lighting -> Camera Angle -> Censorship Fixes " & _
...
End Sub
' =========================================================================
' メモリ上の ConfigData を JSONファイルとして保存する（トランザクション処理・手動リトライ完全版）
' =========================================================================
...
TrySaveAgain: ' ★手動リトライ用のジャンプ地点
    ...
    Do While currentRetry <= maxRetries And Not isSaved
        ...
        ' =================================================================
        ' 【トランザクション処理開始】
        ' ① 現在の正常なファイルを一時的にバックアップ (.bak) として複製
        ...
        ' ② ファイルの書き出し
        Call WriteUTF8File(jsonPath, jsonText)
        
        ' エラーチェック
        If Err.Number = 0 Then
            ' ③ 無事に書き込みが成功したら、一時バックアップを削除
            If Dir(backupPath) <> "" Then Kill backupPath
            isSaved = True
        Else
            ' ④ 失敗時：破損したファイルを消し、.bakから復元する（ロールバック）
            ...
            If currentRetry <= maxRetries Then Call Sleep(500)
        End If
        On Error GoTo 0
    Loop
