...
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} MainWindow 
   Caption         =   "KENZEN SeaArt Helper v4.0.0"
   ClientHeight    =   6050
   ClientLeft      =   110
   ClientTop       =   450
   ClientWidth     =   9740.001
   OleObjectBlob   =   "MainWindow_v4.0.0.frx":0000
...
Private Sub btnPromptSort_Click()
    ...
    ' 最新のデータベース範囲（19行目スタート、B列～BT列）
    mainData = wsMain.Range("B19:BT500").Value
    ...
    For c = 1 To UBound(mainData, 2) Step 2
        ' F列（配列のインデックス3）はBREAK構文前提のため除外する
        If c = 5 Then GoTo NextMainCol
...
Private Sub btnSendLoRAtoCockpit_Click()
    ...
    For i = 0 To UBound(rawElements)
        ...
        If cleanElement <> "" Then
            ...
            ' =========================================================================
            ' ★ 追加する要素同士を結合する際の「半角スペース」ルールを強制適用
            ' =========================================================================
            If Not isDuplicate Then
    ...
    Else
        ' ここは元々実装されていた、既存テキストとの結合時の半角スペースルール
...
' ========================================================
    ...
    If Me.chkSupriseMe.Value = True Then
        ...
        ' 【全モード共通確定枠】
        Call DrawWord(2, 1#, wordsList)    ' (2)キャラ数
        Call DrawWord(21, 1#, wordsList)   ' (21)場所
        Call DrawWord(35, 1#, wordsList)   ' (35)アングル
        
        ' 【松竹梅モード別抽出】
        If Me.opbSFW.Value = True Then
            ' 梅 (SFW)
            Call DrawWord(24, 1#, wordsList)    ' (24)一般的行為
            If Rnd < 0.5 Then Call DrawWord(13, 1#, wordsList) Else Call DrawWord(15, 1#, wordsList)   ' (13)職業 or (15)服装
            Call DrawWord(8, 0.5, wordsList)    ' (8)髪の長さ(50%)
            Call DrawWord(11, 0.5, wordsList)   ' (11)髪の色(50%)
            Call DrawWord(17, 0.3, wordsList)   ' (17)ヘッドウェア(30%)
            Call DrawWord(19, 0.3, wordsList)   ' (19)足元周り(30%)
            Call DrawWord(20, 0.3, wordsList)   ' (20)アクセサリー(30%)
            Call DrawWord(31, 0.5, wordsList)   ' (31)表情(50%)
            Call DrawWord(33, 0.2, wordsList)   ' (33)その他アイテム(20%)
            
        ElseIf Me.opbNSFW.Value = True Then
            ' 竹 (NSFW)
            Call DrawWord(26, 1#, wordsList)    ' (26)性的行為
            If Rnd < 0.5 Then Call DrawWord(14, 1#, wordsList) Else Call DrawWord(16, 1#, wordsList)   ' (14)下着 or (16)服の状態
            Call DrawWord(23, 0.8, wordsList)   ' (23)体位(80%)
            Call DrawWord(29, 0.5, wordsList)   ' (29)身体の部位(50%)
            Call DrawWord(30, 0.5, wordsList)   ' (30)行為の状態(50%)
            Call DrawWord(12, 0.4, wordsList)   ' (12)体毛(40%)
            Call DrawWord(32, 0.5, wordsList)   ' (32)体液(50%)
            Call DrawWord(31, 0.8, wordsList)   ' (31)表情(80%)
            
        ElseIf Me.opbHardcore.Value = True Then
            ' 松 (Hardcore)
            Call DrawWord(27, 1#, wordsList)    ' (27)ボンデージ行為
            Call DrawWord(25, 0.8, wordsList)   ' (25)性的行為大分類(80%)
            Call DrawWord(28, 0.9, wordsList)   ' (28)手段・道具(90%)
            Call DrawWord(16, 0.8, wordsList)   ' (16)服の状態(80%)
            Call DrawWord(23, 0.8, wordsList)   ' (23)体位(80%)
            Call DrawWord(32, 0.8, wordsList)   ' (32)体液(80%)
            Call DrawWord(31, 0.9, wordsList)   ' (31)表情(90%)
        End If
...
Private Sub InitCategoryJump()
    ...
    dataStr = dataStr & ";画風(Art Style),ArtStyle"
...
Private Sub DrawWord(colLogicalIndex As Integer, probability As Double, ByRef outList As Object)
    ...
    ' ★修正1：1行目～18行目はヘッダや凡例エリアのため、19行目以降にデータが無ければ終了
    If lastRow < 19 Then Exit Sub
