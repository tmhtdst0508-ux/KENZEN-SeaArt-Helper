...
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} MainWindow 
   Caption         =   "KENZEN SeaArt Helper v3.1.3"
   ClientHeight    =   6050
   ClientLeft      =   110
   ClientTop       =   450
   ClientWidth     =   9740.001
   OleObjectBlob   =   "MainWindow_v3.1.3.frx":0000
...
Public Sub UpdateLoRAUIState()
    ...
    Call SetControlUIState(Me.btnSaveAsPreset, (isUse And hasPreviewText), &HFFE1C8)
End Sub
...
Private Sub btnAllReset_Click()
    ...
    ' =========================================================================
    userAns = MsgBox("【絶対警告】すべての設定、登録したLoRA、お気に入り、プリセットを完全に消去し、初期状態にリセットしますか？" & vbCrLf & _
                     "※この操作は取り消せません（Undo不可、データは物理的に上書きされます）。" & vbCrLf & vbCrLf & _
                     "[CRITICAL WARNING] Are you absolutely sure you want to completely erase all configuration, registered LoRAs, Favorites, and presets, and reset them to the factory default?" & vbCrLf & _
    ...
    MsgBox "設定ファイルを初期状態（v3.0.x 既定値）に完全リセットしました！" & vbCrLf & _
           "メモリとJSONファイルの同期は完了しています。" & vbCrLf & _
           "画面表示を完全にクリーンアップするため、一度このメインウィンドウを閉じ、開き直すことをお勧めします。" & vbCrLf & vbCrLf & _
           "System configuration has been completely reset to the factory default!" & vbCrLf & _
...
'-------------------------------------------------------------------------
' [Call Preset] ボタン（JSON対応版：選択したプリセット内容を完全復元・欠落検知版）
'-------------------------------------------------------------------------
...
Private Sub btnCallLoRAPreset_Click()
    ...
    Dim i As Integer ' ★追加：リスト選択ループ用の変数
    ...
    If foundCount > 0 Then
        ...
        ' ========================================================
        ' ★バグ修正：Wrapを呼ぶ前に、リストの全項目を強制的に「選択状態」にする
        ' ========================================================
        For i = 0 To Me.lstSelectedLoRA.ListCount - 1
            Me.lstSelectedLoRA.Selected(i) = True
        Next i
        
        ' 無事に1件でも読み込めたらWrapしてプレビュー生成
        Call btnWrapLoRA_Click
        Application.StatusBar = "● Preset '" & presetName & "' loaded."
        
        ' もし欠落したLoRAがあった場合は、警告ダイアログを出す
        If missingCount > 0 Then
            MsgBox "プリセットの一部（" & missingCount & "件）は、大元のLoRAデータが削除されているため除外されました。" & vbCrLf & _
                   "Some LoRAs (" & missingCount & " items) were excluded because their original data was deleted.", _
...
Private Sub btnDoImport_Click()
    ...
    ' --- LoRA Base Data（★固有ネガティブ・鉄壁互換マージ実装） ---
            ...
            For Each impItem In importDict("Fav_List")
                ...
                If Not isDup Then
                    If ConfigData("Fav_List").Count < 50 Then
                        ...
                        cntFav = cntFav + 1
                    Else
                        ' ★上限到達！こぼれたデータをサルベージ用コレクションに退避
                        overflowFavCol.Add impItem
                    End If
                End If
            Next impItem
        Else
            ' 上書き（Replace All）の場合も、50件目までは入れて、残りはサルベージへ
    ...
    ' --- Negative Stock (ネガティブストック) のインポート ---
    If Me.chkImportNegaStock.Value And importDict.Exists("NegativeStock") Then
        If modeIsMerge Then
            ...
            Dim beforeCount As Long ' ★追加：マージ前の件数を記憶する変数
            
            ' 1. 既存のストックをカンマで分割し、辞書（tempDict）に登録
            ...
            beforeCount = tempDict.Count ' ★追加：既存のタグ数を記憶
            
            ' 2. インポート側のストックも同様に分割して追加
            ...
            ' ★追加：純粋に新しく追加されたタグの数を計算
            cntNegaStock = tempDict.Count - beforeCount
            
            ' 3. 重複が排除された綺麗なタグ群を、再びカンマとスペースで結合して保存
    ...
    If Me.chkImportNegaStock.Value Then reportMsg = reportMsg & vbCrLf & "- Negative Stock: " & cntNegaStock & " タグ" ' ★追加
    ...
    ' ★修正：cntNegaStock = 0 も条件に追加
    If modeIsMerge And cntPosi = 0 And cntNega = 0 And cntNegaStock = 0 And cntLoRAPreset = 0 And cntLoRA = 0 And cntFav = 0 Then
...
'-------------------------------------------------------------------------
' [Prompt Sort] ボタン：v3.0.6 最新版（セットタグ完全保護＆正規表現PACKエンジン搭載）
'-------------------------------------------------------------------------
Private Sub btnPromptSort_Click()
    ...
    prepData = wsMain.Range("A14:ZZ16").Value
    Dim pc As Long
    ...
    ' "NOT" をスキップ対象に含める（1セットとして扱い、2単語目でジャッジするため）
    prepDict("NOT") = True
    ...
    ' 最新のデータベース範囲（19行目スタート、B列～BR列）
    mainData = wsMain.Range("B19:BR500").Value
    ...
    For c = 1 To UBound(mainData, 2) Step 2
        ' D列（配列のインデックス3）はBREAK構文前提のため除外する
        If c = 3 Then GoTo NextMainCol
        ...
        For r = 1 To UBound(mainData, 1)
            If Not IsError(mainData(r, c)) And Not IsEmpty(mainData(r, c)) Then
                ...
                If exactDWord <> "" Then
                    ...
                    ' ★修正の要：カンマを含むフレーズ（PACK用）は、ハイフンを破壊する前のオリジナル状態を保存する
                    ...
                    If exactDWord <> "" Then
                        ...
                        ' ★修正：オリジナル状態の文字列（O-legged, knees apart）をPACK用リストに登録
...
NextMainCol:
    ...
    For pIndex = 0 To UBound(paragraphArr)
        ...
        ' 3. カンマの正規化とブラケット除去（パックした「後」に実行する）
        regEx.Pattern = "\s*,\s*"
            ...
            If rawChunk <> "" Then
                ...
                If Not isDuplicate Then
                    ...
                    If cleanStr <> "" Then
                        ...
                        Else
                            ' ========================================================
                            ' ★フレーズ内全探索スコア継承ロジック（"and"等の連結・助詞スキップに完全対応）
                            ' ========================================================
                            ...
                            If UBound(subParts) > 0 Then
                                ...
                                ' フレーズ内のすべての連続する単語の組み合わせを網羅的にテストする
                                For startIdx = 0 To UBound(subParts)
                                    ...
                                    For endIdx = startIdx To UBound(subParts)
                                        ...
                                        ' フレーズ全体と完全一致するものは既に上で評価済みなので除外
                                        If StrComp(tryStr, cleanStr, vbTextCompare) <> 0 Then
                                            ' ★ tryStr が単なる助詞・前置詞（and, in, on, NOT 等）の場合はスコアとして拾わない
                                            If Not prepDict.Exists(tryStr) Then
                                                If exactDict.Exists(tryStr) Then
                                                    ' 登録されているタグの中で、一番優先度が高い（列番号が若い＝数値が小さい）スコアを採用
...
'-------------------------------------------------------------------------
' [Save As Preset] ボタン（JSON対応：重複・上書き対応版）
'-------------------------------------------------------------------------
Private Sub btnSaveAsPreset_Click()
    Dim presetName As String
    Dim i As Long
...
Private Sub btnSendLoRAtoCockpit_Click()
    ...
    For i = 0 To UBound(rawElements)
        ...
        If cleanElement <> "" Then
            ...
            If Left(cleanElement, 6) = "<lora:" Then
                ' ウェイト変更対策：名前部分（またはハッシュ部分）だけで重複をジャッジする
                parts = Split(cleanElement, ":")
                If UBound(parts) >= 1 Then
                    loraBase = parts(0) & ":" & parts(1)
                Else
                    loraBase = cleanElement
                End If
                
                If InStr(1, currentMain, loraBase & ":", vbTextCompare) > 0 Or _
                   InStr(1, currentMain, loraBase & ">", vbTextCompare) > 0 Then
                    isDuplicate = True
                End If
            Else
                ' トリガーワードの厳密な境界チェック（特殊記号の完全エスケープ）
            ...
            ' =========================================================================
            ' ★ 修正の要：追加する要素同士を結合する際の「半角スペース」ルールを強制適用
            ' =========================================================================
            If Not isDuplicate Then
                If appendedPrompt = "" Then
                    appendedPrompt = cleanElement
                Else
                    ' 直前がLoRAタグ(>)で終わり、次に追加するものもLoRAタグ(<lora:)である場合
                    If Right(appendedPrompt, 1) = ">" And Left(cleanElement, 6) = "<lora:" Then
    ...
    Else
        ' ここは元々実装されていた、既存テキストとの結合時の半角スペースルール（完璧です）
        If Right(currentMain, 1) = ">" And Left(appendedPrompt, 1) = "<" Then
            Me.txtMain.text = currentMain & " " & appendedPrompt
        ElseIf Right(currentMain, 1) = "," Then
            Me.txtMain.text = currentMain & " " & appendedPrompt
        Else
            Me.txtMain.text = currentMain & ", " & appendedPrompt
        End If
...
Private Sub btnSendLoRAtoFav_Click()
    ...
    For i = 0 To UBound(rawElements)
        ...
        If cleanElement <> "" Then
            ...
            If Left(cleanElement, 6) = "<lora:" Then
                ...
                If InStr(1, currentFav, loraBase & ":", vbTextCompare) > 0 Or _
                   InStr(1, currentFav, loraBase & ">", vbTextCompare) > 0 Then
                    isDuplicate = True
                End If
            Else
                Dim escWord As String
    ...
    Else
        ...
        ElseIf lastChar = ">" And Left(appendedPrompt, 1) = "<" Then
...
'-------------------------------------------------------------------------
' [ btnWrapBracket ] ボタン：選択した文字列を角括弧 [] でくくる（トグル式）
'-------------------------------------------------------------------------
Private Sub btnWrapBracket_Click()
    ...
    If targetTextBox.SelLength > 0 Then
        ...
        If Left(selectedStr, 1) = "[" And Right(selectedStr, 1) = "]" Then
            ' 中身だけを取り出す
            Dim unwrapped As String
            unwrapped = Mid(selectedStr, 2, Len(selectedStr) - 2)
            
            targetTextBox.SelText = unwrapped
            
            ' 解除後も選択状態を維持
            targetTextBox.SelStart = oldStart
            targetTextBox.SelLength = Len(unwrapped)
        Else
            ' [] でくくる
            Dim wrapped As String
            wrapped = "[" & selectedStr & "]"
            
            targetTextBox.SelText = wrapped
            
            ' くくった後も全体を選択状態にしておく（連続操作しやすいように）
            targetTextBox.SelStart = oldStart
            targetTextBox.SelLength = Len(wrapped)
        End If
...
Private Sub btnSebdToFav_Click()
    ' 1. プロンプトの転送処理を実行
    Call TransferPrompt
...
Private Sub UserForm_Initialize()
    ...
    Me.btnWrapBracket.Enabled = False ' ← 追加
...
' =========================================================================
' UI状態の総合更新（Done!ボタンおよび各クリアボタンの連動）
' =========================================================================
Public Sub UpdateDoneButtonState()
    Dim currentText As String
    currentText = Trim(Me.txtMain.text)
    ...
    ' 判定：テキストが空ではなく、かつ（変更フラグが立っている OR 内容が前回コピー時と違う）
    isReady = hasText And (IsDirty Or (currentText <> LastCopiedPrompt))
    
    ' =====================================================================
    ' UIへの反映：Done! ボタン
    ' =====================================================================
    Me.btnDone.Enabled = isReady
    
    If isReady Then
        Me.btnDone.BackColor = colSuccess
        Me.btnDone.ForeColor = vbBlack
    Else
        Me.btnDone.BackColor = colDisabled
        Me.btnDone.ForeColor = &H808080
    End If
    
    ' =====================================================================
    ' UIへの反映：クリアボタン（追加マージ分）
    ' =====================================================================
    ' テキストボックスが空ならDisabled、1文字でもあればEnabledにする
    Me.btnClear.Enabled = hasText
    Me.btnAllClear.Enabled = hasText
    Me.btnWrapBracket.Enabled = hasText ' ← 追加
...
Private Sub btnSearch_Click()
    ...
    ' ※オブジェクト名はご自身の環境に合わせて txtSearch 等に変更してください
    ...
    ' 2. 検索実行
    ' 前回と同じワードなら「次を検索（FindNext）」、新しいワードなら「最初から（Find）」
    If searchText <> lastSearchText Or lastFoundCell Is Nothing Then
        ' 管理セルを気にする必要がなくなったため、シート全体をシンプルに検索
...
If Not foundCell Is Nothing Then
    ...
    ' --- 【修正箇所】右隣へのジャンプ制御 ---
    ' 日本語で検索、かつアクティブシートが「My Favorite」ではない場合のみ右へ飛ぶ
...
Public Sub LockLoRAUIIfEmpty()
    ...
    If isLoRAEmpty And isPresetEmpty Then
        ...
        Me.btnSaveAsPreset.Enabled = False
...
Private Sub btnFeelLucky_Click()
    ...
    If Me.chkSupriseMe.Value = True Then
        ...
        ' 【全モード共通確定枠】
        Call DrawWord(1, 1#, wordsList)    ' (1)キャラ数
        Call DrawWord(16, 1#, wordsList)   ' (16)場所
        Call DrawWord(29, 1#, wordsList)   ' (29)アングル
        
        ' 【松竹梅モード別抽出】
        If Me.opbSFW.Value = True Then
            ' 梅 (SFW)
            Call DrawWord(19, 1#, wordsList)    ' 一般的行為
            If Rnd < 0.5 Then Call DrawWord(9, 1#, wordsList) Else Call DrawWord(11, 1#, wordsList)   ' 職業 or 服装
            Call DrawWord(4, 0.5, wordsList)    ' 髪の長さ(50%)
            Call DrawWord(7, 0.5, wordsList)    ' 髪の色(50%)
            Call DrawWord(13, 0.3, wordsList)   ' ヘッドウェア(30%)
            Call DrawWord(14, 0.3, wordsList)   ' 足元周り(30%)
            Call DrawWord(15, 0.3, wordsList)   ' アクセサリー(30%)
            Call DrawWord(26, 0.5, wordsList)   ' 表情(50%)
            Call DrawWord(28, 0.2, wordsList)   ' その他アイテム(20%)
            
        ElseIf Me.opbNSFW.Value = True Then
            ' 竹 (NSFW)
            Call DrawWord(21, 1#, wordsList)    ' 性的行為
            'Call DrawWord(30, 1#, wordsList)    ' 修正
            If Rnd < 0.5 Then Call DrawWord(10, 1#, wordsList) Else Call DrawWord(12, 1#, wordsList)   ' 下着 or 服の状態
            Call DrawWord(18, 0.8, wordsList)   ' 体位(80%)
            Call DrawWord(24, 0.5, wordsList)   ' 身体の部位(50%)
            Call DrawWord(25, 0.5, wordsList)   ' 行為の状態(50%)
            Call DrawWord(8, 0.4, wordsList)    ' 体毛(40%)
            Call DrawWord(27, 0.5, wordsList)   ' 体液(50%)
            Call DrawWord(26, 0.8, wordsList)   ' 表情(80%)
            
        ElseIf Me.opbHardcore.Value = True Then
            ' 松 (Hardcore)
            Call DrawWord(22, 1#, wordsList)    ' ボンデージ行為
            'Call DrawWord(30, 1#, wordsList)    ' 修正
            Call DrawWord(20, 0.8, wordsList)   ' 性的行為大分類(80%)
            Call DrawWord(23, 0.9, wordsList)   ' 手段・道具(90%)
            Call DrawWord(12, 0.8, wordsList)   ' 服の状態(80%)
            Call DrawWord(18, 0.8, wordsList)   ' 体位(80%)
            Call DrawWord(27, 0.8, wordsList)   ' 体液(80%)
            Call DrawWord(26, 0.9, wordsList)   ' 表情(90%)
        End If
...
Private Sub InitCategoryJump()
    ...
    dataStr = dataStr & ";キャラ数(Character Count),Count"
    dataStr = dataStr & ";キャラの配置(Character Placement),Placement"
    dataStr = dataStr & ";関係性(Relationship),Relationship"
    dataStr = dataStr & ";肌の色・属性(Skin & Attributes),Skin"
    dataStr = dataStr & ";体型(Body Type),BodyType"
    dataStr = dataStr & ";髪の毛全てへのワイルドカード(Wildcard for Hair),WildcardforHair"
    dataStr = dataStr & ";髪の長さ(Hair length),HairLength"
    dataStr = dataStr & ";前髪(Bangs),Bangs"
    dataStr = dataStr & ";髪の結び目(Tying),Tying"
    dataStr = dataStr & ";髪の色(Hair Color),HairColor"
    dataStr = dataStr & ";体毛(Body Hair),BodyHair"
    dataStr = dataStr & ";職業(Occupation),Occupation"
    dataStr = dataStr & ";下着(Underwear),Underwear"
    dataStr = dataStr & ";服装(Outfit),Outfit"
    dataStr = dataStr & ";服の状態(Outfit State),OutfitState"
    dataStr = dataStr & ";ヘッドウェア(Headwear),Headwear"
    dataStr = dataStr & ";手元周り(Hands & Wrists),Wrists"
    dataStr = dataStr & ";足元周り(Footwear & Legwear),Footwear"
    dataStr = dataStr & ";アクセサリー類(Accessories),Accessories"
    dataStr = dataStr & ";場所(Location),Location"
    dataStr = dataStr & ";時間帯・周囲の状況(Time & Surroundings),Time"
    dataStr = dataStr & ";体位(Position),Position"
    dataStr = dataStr & ";一般的行為(General Action),GeneralAction"
    dataStr = dataStr & ";性的行為大分類(Major Categories of Sexual Act),MajorSexualAct"
    dataStr = dataStr & ";性的行為(Sexual Act),SexualAct"
    dataStr = dataStr & ";ボンデージ行為(Bondage Action & Movement),BondageAction"
    dataStr = dataStr & ";手段・道具(Means & Props),Means"
    dataStr = dataStr & ";身体の部位(Body Parts),BodyParts"
    dataStr = dataStr & ";行為の状態(Interaction State),InteractionState"
    dataStr = dataStr & ";表情(Expressions),Expressions"
    dataStr = dataStr & ";体液(Body fluids),BodyFluids"
    dataStr = dataStr & ";その他アイテム(Misc Items),Items"
    dataStr = dataStr & ";光源(Lighting),Lighting"
    dataStr = dataStr & ";アングル(Camera Angle),CameraAngle"
    dataStr = dataStr & ";修正 / その他(Censorship Fixes & Others),Censorship"
...
' colLogicalIndex : 1?30 の論理的な列番号（1ならB列、2ならD列...）
...
Private Sub DrawWord(colLogicalIndex As Integer, probability As Double, ByRef outList As Object)
    ...
    ' ★修正1：1行目?24行目はヘッダや凡例エリアのため、25行目以降にデータが無ければ終了
    ...
    For r = 25 To lastRow
        ...
        If Not IsError(ws.Cells(r, actualCol).Value) Then
            ...
            ' 空白チェック
            If cellVal <> "" Then
                ' ★修正2：文字列に「凡例」や「プロンプト」が含まれている場合は、くじ引きの箱に入れない
                If InStr(1, cellVal, "凡例", vbTextCompare) = 0 And _
                   InStr(1, cellVal, "プロンプト", vbTextCompare) = 0 Then
