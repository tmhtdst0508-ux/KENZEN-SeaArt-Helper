...
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} MainWindow 
   Caption         =   "KENZEN SeaArt Helper v4.1.0"
   ClientHeight    =   6050
   ClientLeft      =   110
   ClientTop       =   450
   ClientWidth     =   9740.001
   OleObjectBlob   =   "MainWindow_v4.1.0.frx":0000
...
Private Sub btnPromptSort_Click()
    ...
    If Not ConfigData Is Nothing Then
        If ConfigData.Exists("LoRA_List") Then
            ...
            For Each loraItem In ConfigData("LoRA_List")
                ...
                ' ★修正：現在のプロンプト内に、このLoRAタグが実際に適用されているかチェックする
                ...
                ' ★適用されている場合のみ、トリガーワードを最前列（0.5）として辞書登録する
    ...
    ' 最新のデータベース範囲（20行目スタート、B列～BZ列）
    mainData = wsMain.Range("B20:BZ500").Value
    ...
    For c = 1 To UBound(mainData, 2) Step 2
        ' H列（配列のインデックス7）はBREAK構文前提のため除外する
...
Private Sub InitCategoryJump()
    ...
    dataStr = dataStr & ";4.キャラの配置(Character Placement),Placement"
    ...
    dataStr = dataStr & ";13.表情(Expressions),Expressions"
    dataStr = dataStr & ";14.体毛(Body Hair),BodyHair"
    dataStr = dataStr & ";15.体位(Position),Position"
    dataStr = dataStr & ";16.一般的行為(General Action),GeneralAction"
    dataStr = dataStr & ";17.性的行為大分類(Major Categories of Sexual Act),MajorSexualAct"
    dataStr = dataStr & ";18.性的行為(Sexual Act),SexualAct"
    dataStr = dataStr & ";19.ボンデージ行為(Bondage Action & Movement),BondageAction"
    dataStr = dataStr & ";20.職業(Occupation),Occupation"
    dataStr = dataStr & ";21.下着(Underwear),Underwear"
    dataStr = dataStr & ";22.服装(Outfit),Outfit"
    dataStr = dataStr & ";23.服の状態(Outfit State),OutfitState"
    dataStr = dataStr & ";24.服の色 (Clothing Color),ClothingColor"
    dataStr = dataStr & ";25.服の素材 (Clothing Material),ClothingMaterial"
    dataStr = dataStr & ";26.ヘッドウェア(Headwear),Headwear"
    dataStr = dataStr & ";27.手元周り(Hands & Wrists),Wrists"
    dataStr = dataStr & ";28.足元周り(Footwear & Legwear),Footwear"
    dataStr = dataStr & ";29.アクセサリー類(Accessories),Accessories"
    dataStr = dataStr & ";30.手段・道具(Means & Props),Means"
    dataStr = dataStr & ";31.身体の部位(Body Parts),BodyParts"
    dataStr = dataStr & ";32.行為の状態(Interaction State),InteractionState"
    dataStr = dataStr & ";33.体液(Body fluids),BodyFluids"
    dataStr = dataStr & ";34.場所(Location),Location"
    dataStr = dataStr & ";35.時間帯・周囲の状況(Time & Surroundings),Time"
    dataStr = dataStr & ";36.その他アイテム(Misc Items),Items"
    dataStr = dataStr & ";37.光源(Lighting),Lighting"
    dataStr = dataStr & ";38.効果(Effects),Effects"
    dataStr = dataStr & ";39.修正 / その他(Censorship Fixes & Others),Censorship"
